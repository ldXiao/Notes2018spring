...
\begin{dfn}
A sheaf $F\in \ssh(\calc,\tau)$ is a \textbf{manifold}\sidenote{{\color{red}In French, both algebraic variety and manifolds are called vari\'et\'e. I suppose it is just a convention.}} (respect to a context $(\calc,\tau,\bfp)$) if there exists a family of objects $\{U_i\}_{i\in I}$ in $\calc$ and a morphism $p:\coprod_{i\in I}h_{U_i}\lrta F$ such that the following two condition is satisfied.
\begin{enumerate}
\item The morphism $p$ is an epimorphism of sheaves.
\item For each $i\in I$, the morphism $h_{U_i}\lrta F$ is an open immersion.
\end{enumerate}
