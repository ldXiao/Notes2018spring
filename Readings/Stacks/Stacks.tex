...
\begin{dfn}\ 
\begin{enumerate}
\item Let $X\in \calc$. A morphism of sheaves $f:F\lrta h_X$ is an \textbf{open immersion} if it is a monomorphism and there exists a family of morphisms $\{X_i\lrta X\}$ in $\bfp$ so that their image under $f$ is the same as the image under\sidenote[][-6\baselineskip]{{\color{red}This ``image'' seems to mean the image of $F(X_i)\lrta F(X)$ under the natural transformation $f$}} $\coprod_i h_{X_i}\lrta h_X$.
...
$$
(F\times_G H)\times_H h_X\simeq F\times_G \times h_X
$$
