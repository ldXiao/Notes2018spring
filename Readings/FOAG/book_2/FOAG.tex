...
Assume $q=0$ and $a=0$, then $f=0$. In this trivial case the support of $f$ is empty set because $fs=0$ for whatever $s\in A$.

In the case $f=q(\bar{x})+a \bar{y}$, $q\neq 0,a\neq 0$ 
$$
\bar{y}^2f=0
$$
which implies $\bar{y}^2\in\scp$ for $[\scp]\in \supp f$. But in fact, all the prime ideals in $A$ has contains $\bar{y}$. 

\underline{Claim}: if $f=q(\bar{x})+a\bar{y}$ with $q\neq 0, a\neq 0$, then $\supp f=\spec A$.

Indeed, for any $\scp_b=(\bar{x}-b,\bar{y})$, the  multiplicative subset $S_b=A-\scp_b$ consists of all $g(\bar{x})+c\bar{y}$ where $g(b)\neq 0$.
\end{proof}
%---------------------------------------------------------------------------------------
