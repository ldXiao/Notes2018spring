...
& I_x/I_x^2\lrta K^n\\
& f\longmapsto \left(\frac{\pd f(x)}{\pd x_j}\right)_{1\leq j\leq n}
\end{aligned}\right.
$$
$f\in I^2_x\Lrta f=\sum_{i,j}h_{ij}(X_i-x_i)(X_j-x_j)$, and in general 
$$
f=\sum_{i}^n(X_i-x_i)\frac{\pd f}{\pd x_i}(x)+I_x^2.
$$
Let $(f_1,...,f_m)$ be a generating set of $I$. Then $(\theta(f_1),...,\theta(f_m))$ are the columns of $J_{\underline{f}}(x)$ and if $f\in I$, 
$$
f=\sum_j g_j f_j
$$
$$
\Lrta \frac{\pd f}{\pd x_i}(x)=\sum^n_{j=1} g_j(x)\frac{\pd f_j}{x_i}(x)
$$
$
\Lrta 
$ the span of $\theta(f_j)$ is $\theta(I_x/I_x^2)$, so 
$$
rank\  J_{\underline{f}}(x)=\dim_K\theta(I/I_x^2)=\dim_K(I/I_x^2)
$$
Consider
$$
...
Then $=rank \ J_{\underline{f}}(x)+\dim_K(I_x/I+I_x^2)$.  so 

$x$ is non-singular iff 
