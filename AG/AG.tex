...
\section{Mar 9th: Continue and Nonsingular varieties}
...
$\Lrta$ $\tilde{f}(\alpha)=0$ where $\tilde{f}=\left(\prod c_i\right)f\in A[T]=K[X_1,...,X_d,T]$. Define $\tilde{Y}=V(\tilde{f})\subset \affn^{d+1}$. It is wat we wanted.
\begin{enumerate}[label=(\arabic*)]
\item $\tilde{Y}$ is a irreducible hypersurfaces
...
\right.
$$
so the fraction fiedl $K(\tilde{Y})$ injects into $K(Y)$. The image of $K(\tilde{Y})$ contains $X_1,...,X_d$ and $\alpha$ spt contains $E(\alpha)$, i.e., $K(\tilde{Y})=K(Y)$
\end{proof}
...
Indeed suppose $\underline{h}=(h_1,..,h_k)$ also generated $I(Y)$ so
$$
...
\end{aligned}\right.
$$
so $X=Y=0$ is the only singular point if $K$ has characteristic $\neq 2,3$.
...
\end{dfn}
\begin{rmk}
$\calo_{Y,y}$ is a local ring, it has a unique maximal ideal which is $\calo_x\calo_{Y,x}$ in the affine case. Moreover $\calo_{Y,x}/\scm/\calo_{Y,x}=K$ by $f\mapsto f(x)$.
\end{rmk}

N.B. Intuitively,  $f\in \calo_{Y,x}$, defined around $x\in\scm_{Y,x}$
$$
f=f(x)+\sum^n_{j=1}\frac{\pd f}{\pd x_j}(x)(X-x_j)
$$
