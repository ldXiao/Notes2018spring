...
\begin{dfn}
$X$ a scheme $U\subset X$ open $(U,\calo_X|U)$ is called an open subscheme of $X$, $j:U\inj  X$ is called an \text{open immersion}.
\end{dfn}
\begin{dfn}
...
\begin{dfn}
A \textbf{closed subscheme} of $X$ is an equivalent class of closed immersions modulo
...
\begin{ex}
Example. 
$(1)$ Let $X=\spec (A)$ let $I\subset A$ ideal and $\spec(A/I)\overset{f}{\lrta}\spec A$ the canonical morphism. Then it is a closed immersion

 We saw thart $\spec A/I\overset{\sim}{\lrta}V(I)\subset \spec(A)$ is a homeomorphism.
...
\section*{Two standard construction of $\proj^n_\intg$}
\begin{enumerate}[label=(\arabic*)]
\item $\mathsf{Proj}$ of a graded ring (hartshorne p 76)
$$
A=\oplus_{d\geq 0} A_d, A_d A_e\subset A_{d+e}
$$
$A_0$ is a ring and each $A_d$ is an $A_0$-algebra.
...
$$
X\overset{\text{closed inclusion}{\inj}}\proj^n_S\lrta S
$$
